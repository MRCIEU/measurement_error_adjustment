/* recodes_23.do - Amends some of the special coded non-numeric values */ 
...
*run for each part file separately
 foreach f in a b c {
   use $RESULTS/mainfile23`f', clear
   ds
...
*-1 only (codings 170, 100696)
   foreach vnum in 129 130 4282 {
     forval inst=2/3 {
        local varname="x`vnum'_`inst'_0"
        capture replace `varname'=0 if `varname'==-1
     }
   }
...
*-1 and -3 and -10 (less than) (codings 100290 100298 100300 100329 100355 100373 100537 100567 100569) 
   foreach vnum in 699 757 777 796 1050 1060 1070 1080 1090 1289 1299 1309 1319 1438 1458 1488 1498 1528 ///
     ...
     forval inst=2/3 {
        local varname="x`vnum'_`inst'_0"
        capture replace `varname'=0.5 if `varname'==-10
        capture replace `varname'=. if `varname'==-1
        capture replace `varname'=. if `varname'==-3
     }
   }

*-1 and -3 (codings 13 100291) 
   foreach vnum in 709 767 874 884 894 904 914 1160 1269 1279 1568 1578 1588 1598 1608 1807 1845 1873 1883 ///
                   2149 2217 2405 2714 2794 2824 2867 2897 2926 2946 2966 2976 3436 3526 3536 3581 3627 ///
...
*-1 and -2 and -3 (codings 100306 100307 100504 100585)
   foreach vnum in 845 864 2139 2744 {
     forval inst=2/3 {
        local varname="x`vnum'_`inst'_0"
        capture replace `varname'=. if `varname'==-1
        capture replace `varname'=. if `varname'==-2
        capture replace `varname'=. if `varname'==-3
     }
   }
...
*-1 and -3 and -6 (coding 100582)
   foreach vnum in 3710 {
     forval inst=2/3 {
        ...
        capture replace `varname'=. if `varname'==-1
        capture replace `varname'=. if `varname'==-3
        capture replace `varname'=. if `varname'==-6
     }
   }
...
*-3 and -4 (coding 100586)
   foreach vnum in 2754 2764 3872 {
     forval inst=2/3 {
        local varname="x`vnum'_`inst'_0"
        capture replace `varname'=. if `varname'==-3
        capture replace `varname'=. if `varname'==-4
     }
   }
...
*-1 and -3 and -11 (coding 100595 100598)
   foreach vnum in 2804 3546 {
     forval inst=2/3 {
        ...
        capture replace `varname'=. if `varname'==-1
        capture replace `varname'=. if `varname'==-3
        capture replace `varname'=. if `varname'==-11
     }
   }
...
*-3 (coding 100584)
 ...
 save $RESULTS/mainfile23`f'.dta, replace
 }
